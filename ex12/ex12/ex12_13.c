/* 2016-11-17 20121275 서정민 ex12_13.c */
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#define BEGIN_LEARNING_RATE_A 0.5 // 전반부 초기 학습율 
#define BEGIN_LEARNING_RATE_B 0.1 // 후반부 초기 학습율
...
#define RANDNUMBER_A (2 + (rand() % 100 / 100.0f)) // 0.00 ~ 0.99 사이의 랜덤 넘버
#define RANDNUMBER_B (rand() % 100 / 100.0f) // 0.00 ~ 0.99 사이의 랜덤 넘버
...
int main()
{
	...
	for (float learnRatio = BEGIN_LEARNING_RATE_A; learnRatio >= LEARNING_MIN_RATIO; learnRatio -= LEARNING_REDUCTION_RATIO) // ratio는 학습 강도
	{
		...
		for (int index0 = 0; index0 < NUMBER_OF_PATTERN; ++index0) // 패턴별 탐색
		{
			...
			vWeight[vMinNum] = vWeight[vMinNum] + (trainingSets[vMinNum].inputData - vWeight[vMinNum]) * learnRatio; // 목표치와의 차이만큼 저장
	...
	for (float learnRatio = BEGIN_LEARNING_RATE_B; learnRatio >= LEARNING_MIN_RATIO; learnRatio -= LEARNING_REDUCTION_RATIO) // ratio는 학습 강도
	{
		...
		for (int index0 = 0; index0 < NUMBER_OF_PATTERN; ++index0) // 패턴별 탐색
		{
			...
			vWeight[vMinNum] = vWeight[vMinNum] + (trainingSets[vMinNum].inputData - vWeight[vMinNum]) * LEARNING_MIN_RATIO; // 목표치와의 차이만큼 저장
			...
			wWeight[vMinNum] = wWeight[vMinNum] + (trainingSets[vMinNum].desiredOutput - wWeight[vMinNum]) * learnRatio; // 목표치와의 차이만큼 저장
